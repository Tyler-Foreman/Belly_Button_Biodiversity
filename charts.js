...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var yticks = otuIds.slice(0,10).map(number => `OTU ${number}`).reverse();
    console.log(yticks)
  ...
  // Use Plotly to plot the data with the layout. 
    ...
    // array for an object in the array whose id property 
    // matches the ID number passed into buildCharts() 
    // function as the argument.
    var metadata = data.metadata;
    var metaArray = metadata.filter(metaObj => metaObj.id == sample); 
    
    // Create a variable that holds the first sample in the array created in Step 2.
